...
<script>
fetch('/parking-layout')
    .then(response => response.json())
    .then(data => {
        // Process the retrieved data and generate parking spots
        generateParkingSpots(data);
    })
    .catch(error => {
        console.error('Error:', error);
    });

function generateParkingSpots(parkingLayoutData) {
    const map = document.getElementById('map');
    map.innerHTML = '';

    parkingLayoutData.forEach(rowData => {
        const rowDiv = document.createElement('div');
        rowDiv.classList.add('row');

        rowData.forEach(spotData => {
            const spotDiv = document.createElement('div');
            spotDiv.classList.add('spot');
            spotDiv.classList.add(spotData.is_available ? 'available' : 'unavailable');
            spotDiv.dataset.row = spotData.row;
            spotDiv.dataset.column = spotData.column;
